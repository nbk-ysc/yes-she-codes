...
(def DataDeCompraValida
  (s/constrained java.time.LocalDate
                 (fn [data]
                   (let [data-atual (time/local-date)]
                     (or (= data data-atual) (time/before? data data-atual))))))

(def EstabelecimentoValido
  (y.util/min-caracteres 2))

(def CategoriaValida
  (s/enum "Alimentação" "Automóvel" "Casa" "Educação" "Lazer" "Saúde"))
...
(defn filtra-compras-por-cartao
  [cartao compras]
...
(def total-gasto-no-mes
  (comp total-gasto filtra-compras-no-mes))

(defn agrupa-gastos-por-categoria
  [compras]
